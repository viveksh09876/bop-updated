...
<div class="faqs row">
    <div class="floatleft mtop10"><h2><?php echo __('Edit Show'); ?></h2></div>
    <?php
    ...
    <table cellspacing="0" cellpadding="7" border="0" align="center">
		<tr>
			...
			<td align="left"><?php	echo $this->Form->input('title',array('class' => 'input required', 'label' => false, 'div' => false, 'style'=>'width: 450px;'));?>
                 ...
                 <?php $showTypes=array('Conformation'=>'Conformation','Agility'=>'Agility','Obedience'=>'Obedience','Rally'=>'Rally','Best in Pedigree'=>'Best in Pedigree','Show Stoppers'=>'Show Stoppers'); ?>
               <tr>
        ...
        <tr>
			...
			<td align="left"><input type="file" name="filename" />
                         ...
                         <?php echo $this->Form->input('old_image',array('type'=>'hidden','value'=>$this->request->data['Show']['image']))?>
                         <?php echo $this->Form->input('id',array('type'=>'hidden','value'=>$this->request->data['Show']['id']))?>
                         <?php if($this->request->data['Show']['image']){?>
                            <tr>
                            <td align="left">&nbsp;</td>
                                            <td align="left"><img src="<?php echo $this->webroot;?>files/shows/<?php echo $this->request->data['Show']['image'] ?>"></td>
                                </tr>
                           <?php } ?>
...
<script>
 $(function(){
  ...
  $('#ShowAdminEditForm').validate();
 
 
  <?php
  
  $selectValues=explode(',',$this->request->data['Show']['breed_id']);
        $breedCount=count($selectValues);
        for($i=0; $i<$breedCount;$i++){?>
           $("#ShowBreedId option[value='<?php echo $selectValues[$i] ?>']").attr("selected", "selected");
        <?php } ?>
