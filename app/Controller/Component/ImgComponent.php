...
class ImgComponent extends Component
{
	...
	*	$thumbscalew: the maximum width that you want your thumbnail to be resized to
	...
	*/	
	function upload_image_and_thumbnail($data, $maxw, $maxh, $thumbscalew, $thumbscaleh, $folderName,$ext=null) {

		if (strlen($data['name'])>4){ 
					$error = 0;
					$tempuploaddir = "files/temp"; // the /temp/ directory, should delete the image after we upload
					...
					if($ext){
					$extuploaddir = "files/".$folderName."/ext"; 
					}
					...
					if($ext){
					if(!is_dir($extuploaddir)) mkdir($extuploaddir,0755,true);
					}
					...
					if($ext){
					$croppedextfile = $extuploaddir . "/$filename";
					}
                    ...
                    {                    
                        ...
                        if (!copy($data['tmp_name'],"$tempfile"))
                        {
							 ...
							 */
							$this->resizeImage('resize', $tempuploaddir, $filename, $biguploaddir, $filename, $maxw, $maxh, 99);							
							...
							if($ext){
							$this->resizeImage('resizeCrop', $tempuploaddir, $filename, $extuploaddir, $filename, 381, 240, 85);
							}
