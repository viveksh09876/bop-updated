...
class ImageComponent extends Component {
	...
	*/
	 ...
	 * @return $errors
	 */
	function resize($src, $dim1, $dim2 = false, $thumb_subloc, $dim_setting = RESIZE_AUTO, $rename_flag = true)
	{
		$errors = array();
		...
		else 
		{
			...
			* */
			...
			$vertical_shrink_ratio = $width > $dim1 ? $dim1/ $width : 1;
			$horizontal_shrink_ratio = $height > $dim2 ? $dim2 / $height : 1;
			$shrink_ratio = min($vertical_shrink_ratio, $horizontal_shrink_ratio);
			
			$newWidth = $width * $shrink_ratio;
			$newHeight = $height * $shrink_ratio;
