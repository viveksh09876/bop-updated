...
class UsersController extends AppController {
     ...
     */
    public $name = 'Users';
    public $components = array('Uploader', 'Img', 'Core');
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow(array('registration', 'register', 'verify', 'login', 'facebook', 'twitter', 'linkedin', 'twitter_callback',
   ...
   $this->set("banners",$this->ManageBanner->find("first"));
        ...
        if (!empty($_FILES['shop_banner']['name'])) {
            ...
            $this->unlink_thumbs(UPLOAD_KENNELBANNER_DIR, 'ManageBanner', 'shop_banner', array(), array(), 'kennel_banners');
            ...
            if ($this->Upload->do_upload('shop_banner')) {
                ...
                $userArr['shop_banner'] = "'" . $imgdata_arr['file_name'] . "'";
                $ManageBanner['shop_banner'] = $imgdata_arr['file_name'];
            } else {
        ...
        if (!empty($_FILES['vet_banner']['name'])) {
            $this->unlink_thumbs(UPLOAD_KENNELBANNER_DIR, 'ManageBanner', 'vet_banner', array(), array(), 'kennel_banners');
            $config['upload_path'] = UPLOAD_KENNELBANNER_DIR;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = 2165466;
            $config['encrypt_name'] = true;
            ...
            if ($this->Upload->do_upload('vet_banner')) {
                ...
                $userArr['vet_banner'] = "'" . $imgdata_arr['file_name'] . "'";
                $ManageBanner['vet_banner'] = $imgdata_arr['file_name'];
            } else {
                $errors[] = $this->Upload->display_errors();
                $error_flag = true;
            }
        ...
        if (!empty($_FILES['adoption_banner']['name'])) {
            $this->unlink_thumbs(UPLOAD_KENNELBANNER_DIR, 'ManageBanner', 'adoption_banner', array(), array(), 'kennel_banners');
            $config['upload_path'] = UPLOAD_KENNELBANNER_DIR;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = 2165466;
            $config['encrypt_name'] = true;
            ...
            if ($this->Upload->do_upload('adoption_banner')) {
                $imgdata_arr = $this->Upload->data();

               
                $ManageBanner['adoption_banner'] = $imgdata_arr['file_name'];
            } else {
                $errors[] = $this->Upload->display_errors();
                $error_flag = true;
            }
        } 
        if (!empty($_FILES['job_banner']['name'])) {
            $this->unlink_thumbs(UPLOAD_KENNELBANNER_DIR, 'ManageBanner', 'job_banner', array(), array(), 'kennel_banners');
            $config['upload_path'] = UPLOAD_KENNELBANNER_DIR;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = 2165466;
            $config['encrypt_name'] = true;
            ...
            if ($this->Upload->do_upload('job_banner')) {
                $imgdata_arr = $this->Upload->data();

               
                $ManageBanner['job_banner'] = $imgdata_arr['file_name'];
            } else {
                $errors[] = $this->Upload->display_errors();
                $error_flag = true;
            }
        } 
        if (!empty($_FILES['show_banner']['name'])) {
            $this->unlink_thumbs(UPLOAD_KENNELBANNER_DIR, 'ManageBanner', 'show_banner', array(), array(), 'kennel_banners');
            $config['upload_path'] = UPLOAD_KENNELBANNER_DIR;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = 2165466;
            $config['encrypt_name'] = true;
            ...
            if ($this->Upload->do_upload('show_banner')) {
                $imgdata_arr = $this->Upload->data();

               
                $ManageBanner['show_banner'] = $imgdata_arr['file_name'];
            } else {
                $errors[] = $this->Upload->display_errors();
                $error_flag = true;
            }
        ...
        $jai=0;
        if (!empty($ManageBanner)) {
            $ManageBanner["id"] = 1;
         

            //  pr($ManageBanner);
            $this->ManageBanner->save($ManageBanner);
            $jai=1;
        }
        if (!empty($userArr)) {
            $this->User->updateAll($userArr);
            $jai=1;
        }
        if($jai==1){
        $this->Session->setflash("Banner has been Saved");
        $this->redirect(array("action"=>"banners","admin"=>true));
        }
    }
            ...
            if ($this->User->validates(array('fieldList' => array('email')))) {
                ...
                if (!empty($_FILES['filename']['name'])) {
                    ...
                    if ($this->Upload->do_upload('filename')) {
                        $imgdata_arr = $this->Upload->data();
                        $data['User']['photo'] = $imgdata_arr['file_name'];
                    } else {
                        $errors[] = $this->Upload->display_errors();
                        $error_flag = true;
                    }
                }
                $password = $this->Core->generatePassword();
                $data['User']['password'] = $this->Auth->password($password);
                if ($this->User->save($data)) {
                    if (!empty($_FILES['filename']['name']) && $error_flag == false) {
                        $this->create_all_thumbs($imgdata_arr['file_name'], UPLOAD_USERIMG_DIR, 'User', 'filename', array(), 'user_images');
                    }
                    $fname = $data['User']['first_name'];
                    $email = $data['User']['email'];
                    $pwd = $password;
                    /* send password to user---- */
                    $content = "Dear {$fname},<br>You are successfully registered at bestofpedigree.com<br>Please Login with details:<br>Email:{$email}<br>Password: {$password} ";
                    ...
                    $this->Session->setFlash(__('User details have been saved successfully'), 'default', array(), 'success');
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->User->validates(array('fieldList' => array('email')))) {
                ...
                if ($_FILES['filename']['name']) {
                    $this->unlink_thumbs(UPLOAD_USERIMG_DIR, 'User', 'photo', array('id' => $id), array(), 'user_images');
                    $config['upload_path'] = UPLOAD_USERIMG_DIR;
                    $config['allowed_types'] = 'gif|jpg|png|jpeg';
                    $config['max_size'] = 1200;
                    ;
                    ...
                    $data['User']['photo'] = $data['User']['old_image'];
                }

                if ($this->User->save($data)) {
                    if (!empty($_FILES['filename']['name'])) {
                        $this->create_all_thumbs($imgdata_arr['file_name'], UPLOAD_USERIMG_DIR, 'User', 'photo', array(), 'user_images');
                    }
                    $this->Session->setFlash(__('User details have been updated successfully'), 'default', array(), 'success');
                    $this->redirect(array('controller' => 'users', 'action' => 'manage', 'admin' => true));
                }
            } else {
                $errors = $this->User->validationErrors;
    ...
    public function login() {
        $this->Session->delete('rgError');
        if ($this->Auth->login()) {
			
            if (isset($this->request->data['User']) && isset($this->request->data['User']['remember_me']) && $this->request->data['User']['remember_me'] == 1) {
            ...
            if (!empty($this->user)) {
                ...
                if ($this->user['status'] == STATUS_ACTIVE) {
                    if ($this->user['role'] == UserRoleConst::RoleUser) {
                        ...
                        $this->redirect(array('controller' => 'kennel', 'action' => 'index'));
                        //$this->redirect(array('controller' => 'users', 'action' => 'world_map'));
                    } else {
                        $loginError = '1';
                        //$this->Session->destroy();
                        $this->Session->setFlash(__('This account is not valid'), 'default', array(), 'error');
        ...
        //$this->Session->write('loginError', $loginError);
    ...
    public function registration() {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->User->validates(array('fieldList' => array('email')))) {
                ...
                $data['User']['password'] = $this->Auth->password($data['User']['password']);
                $data['User']['kennel_xp'] = '2500';
                $data['User']['funds'] = '2500';
                $data['User']['credits'] = '5000';
                $data['User']['kennel_spaces'] = '5';
                $data['User']['kennel_name'] = $this->request->data['User']['kennel_name'];
                //;=$this->Auth->password($password);
                if ($userData = $this->User->save($data)) {
                    ...
                    $rgError = '1';
                    $data = $this->User->findById($userData['User']['id']);
                    $this->Auth->login($data['User']);
                    $this->redirect(array('controller' => 'kennels', 'action' => 'index'));
                    $this->Session->setFlash(__('Registration has been successfully completed.'), 'default', array(), 'registration_success');
                    /* -----Redirect user to kennel page------ */
                }
                ...
                $this->Session->setFlash(__($err), 'default', array(), 'registration_error');
            }
        }
        $this->Session->write('rgError', $rgError);
        $this->redirect($this->referer());
    }
