...
{
	public $uses = array('User','Breed','Pet','BackgroundImage','BreedingRibbon','EmployerLicence','RetirementMedal','EnergyBone','KennelSpace','GameBreed','UserKennelSpace','UserLicence','UserBreedingRibbon','UserEnergyBone','UserRetirementMedal','UserBgImage');
	 ...
	 */
     function shop(){
         ...
         $this->set('BitechesNotInHeat',$this->GameBreed->find('all',array('conditions'=>array('GameBreed.gender'=>'Bitch','GameBreed.is_in_heat'=>'0','GameBreed.user_id'=>$userId))));
            ...
            /* Emploer Licence buy starts-------*/
             ...
             if($_POST['breeding_ribbon']){
                ...
                if($credit>=$creditsToBuy){
                     $credit=$credit-$creditsToBuy;
                     $data=array();
                     $this->request->data['UserBreedingRibbon']['user_id']=$userId;
                     $this->request->data['UserBreedingRibbon']['cost']=$creditsToBuy;
                     $this->request->data['UserBreedingRibbon']['purchased_date']=date('Y-m-d H:i:s');
                     if($this->UserBreedingRibbon->save($this->request->data)){
                         $this->User->id=$userId;
                         $this->User->saveField('credits',$credit);    //update credits
                         $userUpdatedData=$this->User->findById($userId);
                         $this->Auth->login($userUpdatedData['User']); //update session data
